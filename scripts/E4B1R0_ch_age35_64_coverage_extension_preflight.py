...
if "E3B4C2_FIRST_BRR_EXECUTION=PASS" not in brr_audit:
    raise RuntimeError(
        "missing BRR audit invariant=E3B4C2_FIRST_BRR_EXECUTION=PASS"
    )

# E3B4C2 execution audit attests the execution PASS. The exact replicate
# count is an engine-contract property and was already validated above from
# the frozen E3B4C1 exact BRR engine contract.
if brr["replicate_count"] != 44:
    raise RuntimeError(
        "frozen E3B4C1 BRR replicate count is not 44"
    )
