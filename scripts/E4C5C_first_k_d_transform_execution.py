...
def resolve(headers,kind):
    ...
    exact_est={"ESTIMATE","POINT_ESTIMATE","POINT","THETA","MEAN","MI_ESTIMATE","COMBINED_ESTIMATE","ESTIMATE_MI","VALUE"}
    est=[h for h,n in nh.items() if n in exact_est]
    if not est:
        est=[
            h for h,n in nh.items()
            if ("ESTIM" in n or n.endswith("_MEAN") or n=="MEAN")
            and not any(x in n for x in ["SE","STD","VAR","CI","LOW","HIGH","LCL","UCL","DIFF","RATIO"])
        ]
...
def row_component(row,struct_cols):
    ...
    for k,v in row.items():
        if k in struct_cols:continue
        s=str(v).strip().upper()
        if s=="" : continue
        tokens.append(s)
    joined=" | ".join(tokens)
    norms={re.sub(r"[^A-Z0-9]+","_",x).strip("_") for x in tokens}
    ...
    # Sensitivity names alone may not classify as primary.
    if k and any(x in norms for x in {"LIQ","EQUITY","RETQLIQ"}) and not any(x in norms for x in {"FIN","K_FIN","K_FIN_MEAN"}):
        k=False
    if d and "DEBT2INC" in norms and "PIRTOTAL" not in norms and "PIRTOTAL" not in joined:
        d=False

    if k and d:
        raise RuntimeError(f"ambiguous K+D primary classification in row tokens={tokens}")
...
def execute():
    ...
    with REP.open("r",encoding="utf-8-sig",newline="") as f:
        ...
        for idx,row in enumerate(reader,start=2):
            ...
            if comp is None:continue
            age=map_age(row[rm["age_col"]]);ten=map_tenure(row[rm["tenure_col"]])
            if age is None or ten is None:continue
            value=finite_float(row[rm["estimate_col"]])
            if value is None:
                raise RuntimeError(f"nonfinite primary replicate estimate row {idx}")
            if comp=="K" and value<0:
                raise RuntimeError(f"K replicate domain fail row {idx}: {value}")
