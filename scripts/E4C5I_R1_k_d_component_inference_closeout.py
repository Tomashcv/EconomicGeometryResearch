...
def is_12dp_string(x):
    return re.fullmatch(r"-?\d+\.\d{12}",str(x).strip()) is not None
...
def combined_interval(imp,samp):
...
def difference_interval(renter,owner):
...
with G_INF.open("r",encoding="utf-8",newline="") as fh:
    ...
    for r in rd:
        ...
        serialized_fields=["point_estimate_state","imputation_variance_state","sampling_replicate_mean_state","sampling_variance_state","combined_variance_state","combined_se_state"]
        if not all(is_12dp_string(r[name]) for name in serialized_fields):
            raise RuntimeError(f"cell serialization format {key}")
        row={
            "year":r["year"],
        ...
        if not intervals_overlap(combined_interval(row["imp_var"],row["samp_var"]),interval(row["comb_var"])):
            raise RuntimeError(f"cell combined variance serialization interval {key}")
        if not intervals_overlap(sqrt_interval(row["comb_var"]),interval(row["se"])):
            raise RuntimeError(f"cell SE serialization interval {key}")
...
with H_INF.open("r",encoding="utf-8",newline="") as fh:
    ...
    for r in rd:
        ...
        serialized_fields=["owner_point_state","renter_point_state","renter_minus_owner_point_state","imputation_variance_state_contrast","sampling_replicate_mean_difference_state","sampling_variance_state_contrast","combined_variance_state_contrast","combined_se_state_contrast"]
        if not all(is_12dp_string(r[name]) for name in serialized_fields):
            raise RuntimeError(f"contrast serialization format {key}")
        row={
            "year":r["year"],
            "statistic_id":r["statistic_id"],
        ...
        if not intervals_overlap(combined_interval(row["imp_var"],row["samp_var"]),interval(row["comb_var"])):
            raise RuntimeError(f"contrast combined variance serialization interval {key}")
        if not intervals_overlap(sqrt_interval(row["comb_var"]),interval(row["se"])):
            raise RuntimeError(f"contrast SE serialization interval {key}")
        renter_point=cells[(key[0],key[1],"RENTER")]["point"]
        owner_point=cells[(key[0],key[1],"OWNER")]["point"]
        if not intervals_overlap(difference_interval(renter_point,owner_point),interval(row["point"])):
            raise RuntimeError(f"contrast point serialization interval {key}")
        if row["imp_var"]<0 or row["samp_var"]<0 or row["comb_var"]<0 or row["se"]<0:
...
hard=[
    ...
    ("TWELVE_DECIMAL_SERIALIZATION_FORMAT_PASS",1),
    ("CELL_VARIANCE_QUANTIZATION_INTERVAL_PASS",1),
    ("CELL_SE_QUANTIZATION_INTERVAL_PASS",1),
    ("CONTRAST_VARIANCE_QUANTIZATION_INTERVAL_PASS",1),
    ("CONTRAST_SE_QUANTIZATION_INTERVAL_PASS",1),
    ("CONTRAST_POINT_QUANTIZATION_INTERVAL_PASS",1),
...
log="\n".join([
    ...
    "CONTRAST_SE_QUANTIZATION_INTERVAL_PASS=1",
    "CONTRAST_POINT_QUANTIZATION_INTERVAL_PASS=1",
    "SERIALIZATION_VALIDATION_RULE=12DP_QUANTIZATION_INTERVAL",
